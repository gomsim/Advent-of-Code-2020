...
public class ReportReparatorPart2{
    ...
    public static void main(String[] args){
        ...
        for (int i = 0; i < report.length; i++){
            ...
            int[] components = findTwo(GOAL - report[i]);
            if (components != null)
                System.out.println(current + " " + components[0] + " " + components[1] + " = " + (current+components[0]+components[1]) + " \t-> " + (current*components[0]*components[1]));
        }
    }

    private static int[] findTwo(int subGoal){
        for (int i = 0; i < report.length; i++){
            int current = report[i];
            if (subGoal - current > 0){
                int compInd = binarySearch(subGoal - current, report);
                if (compInd > 0)
                    return new int[] {current, report[compInd]};
            }
        }
        return null;
    }

    private static int binarySearch(int x, int[] array){
        int retInd = -1;
        int searchSpace = array.length/2;
        int i = searchSpace;
        while(array[i] != x && searchSpace > 1){
            searchSpace /= 2;
            if (x < array[i])
                i -= searchSpace;
            if (x > array[i])
